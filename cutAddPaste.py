...
from models.CutAddPaste.trainer.trainer import Trainer, Trainer_DAL 
from models.CutAddPaste.network.model import base_Model, dal_Model 
...
parser.add_argument('--device', default='cuda', type=str,
                    help='cpu or cuda')
parser.add_argument('--use_dal', default=True, type=bool,
                    help='Use DAL-style training loop (requires dal_mode in data generator)') 
...
for idx in tqdm(range(model_num)):
    ...
    # Load Model (use DAL variant when requested)
    if args.use_dal:
        model = dal_Model(configs, device).to(device)
    else:
        model = base_Model(configs, device).to(device)
    print("Data loaded ...")
    if args.use_dal:
        train_in_loader, train_out_loader, val_dl, test_dl, test_anomaly_window_num = data_generator(train_data, test_data, train_labels,
                                                                                                 test_labels, SEED, configs, dal_mode=True)
    else:
    ...
    # Trainer
    if args.use_dal:
        test_score_origin, test_aff, test_rpa_score, test_pa_score, test_pw_score, score_reasonable, predict = Trainer_DAL(model, model_optimizer, train_in_loader,
                                                                                     train_out_loader, val_dl, test_dl, device, configs, idx)
    else:
    ...
    if visualization:
        print('*'*32)
        fig = plt.figure(facecolor="w", figsize=(10, 6), constrained_layout=True)
        ...
        plt.title(time_series_name + '_' + str(idx), pad=14)
        # save figures
        fig.savefig(os.path.join("results", f"{selected_dataset}_seed{SEED}_{idx}_series.pdf"), bbox_inches='tight')
        plt.close(fig)

        # will modify this, so created a backup below and commented it out.
        fig_origin = plt.figure(facecolor="w", figsize=(20, 12), constrained_layout=True)
        ...
        ax_origin.plot(test_score_origin, linewidth=1, color='r')

        # making many many changes here
        # Add time series on a twin axis and shade ground-truth anomalies
        ax2 = ax_origin.twinx()

        # Extract test series (first channel if multivariate)
        t_data, y_data = test_data_plot.index, test_data_plot.values
        if y_data.ndim > 1:
            y_data = y_data[:, 0]

        L_score = len(test_score_origin)
        L_series = len(y_data)

        # Resample series to score-length if needed so x-axes align
        if L_series != L_score:
            x_src = np.arange(L_series)
            x_dst = np.linspace(0, L_series - 1, L_score)
            y_plot = np.interp(x_dst, x_src, y_data)
            t_plot = np.arange(L_score)
        else:
            y_plot = y_data
            t_plot = np.arange(L_series)

        # Plot series on twin axis
        ax2.plot(t_plot, y_plot, color='b', linewidth=1, alpha=0.85)
        # Axis labels (larger fonts)
        ax_origin.set_ylabel('anomaly score', fontsize=30)
        ax2.set_ylabel('value', fontsize=30)

        # Shade ground-truth anomalous intervals
        y_label = test_labels_plot.values
        splits = np.where(y_label[1:] != y_label[:-1])[0] + 1
        splits = np.concatenate(([0], splits, [len(y_label) - 1]))

        if L_series != L_score:
            scale = (L_score - 1) / max(1, (L_series - 1))
            for k in range(len(splits) - 1):
                if y_label[splits[k]]:
                    start = int(round(splits[k] * scale))
                    end = int(round(splits[k + 1] * scale))
                    ax2.axvspan(start, end, color="#ffff00", alpha=0.3)
        else:
            for k in range(len(splits) - 1):
                if y_label[splits[k]]:
                    ax2.axvspan(splits[k], splits[k + 1], color="#ffff00", alpha=0.3)


        # Make both axes' tick labels larger for readability
        ax_origin.tick_params(axis='both', which='major', labelsize=24)
        ax2.tick_params(axis='both', which='major', labelsize=24)

        # Legend with labels for plotted elements
        score_handle = Line2D([0], [0], color='r', linewidth=1, label='anomaly score')
        value_handle = Line2D([0], [0], color='b', linewidth=1, label='value')
        gt_handle = Patch(facecolor="#ffff00", alpha=0.3, label='ground truth anomaly')
        ax_origin.legend(handles=[score_handle, value_handle, gt_handle], loc='upper right', fontsize=22)

        # Ensure title has padding and a much larger font for visibility
        # modify title a little when using aiops
        # plt.title(f"{selected_dataset}_" + time_series_name + '_' + str(idx), pad=16, fontsize=34)
        plt.title(f"{selected_dataset}_" + str(idx), pad=16, fontsize=34)
        # save figures
        fig_origin.savefig(os.path.join("results", f"{selected_dataset}_seed{SEED}_{idx}_score.pdf"), bbox_inches='tight')
...
# visualization
if visualization:
    fig_all = plt.figure(facecolor="w", figsize=(20, 12), constrained_layout=True)
    ax_all = fig_all.add_subplot(111)
    ax_all.plot(detect_list, linewidth=1)
    # save figures
    fig_all.savefig(os.path.join("results", f"{selected_dataset}_seed{SEED}_aggregate_detect_list.pdf"), bbox_inches='tight')
    plt.show()
