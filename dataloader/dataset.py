...
def data_generator(train_data, test_data, train_labels, test_labels, seed, configs, dal_mode=False):
    ...
    train_origin_2 = train_x.copy()

    if ((configs.rate != 0) and (configs.rate <= 1)):
        train_aug_x = cut_add_paste_impute_outlier(train_origin, configs)  
        sample_num = int((1 - configs.discontinuity) * configs.rate * len(train_origin)) 
    ...
    if ((configs.rate != 0) and (configs.rate <= 1)):
        train_aug_x_cap = cut_add_paste_outlier(train_origin_2, configs)
        sample_num = int(configs.discontinuity * (configs.rate) * len(train_origin_2))  
        sample_list = [i for i in range(sample_num)]    
        sample_list = random.sample(sample_list, sample_num)    
        sample = train_aug_x_cap[sample_list, :, :]  
    elif configs.rate > 1:
        train_aug_x_1 = cut_add_paste_outlier(train_origin_2, configs)
        train_aug_x_2 = cut_add_paste_outlier(train_origin_2, configs)
        train_aug_x_cap = np.concatenate((train_aug_x_1, train_aug_x_2), axis=0)
        sample_num = int(configs.rate * len(train_origin_2))
        sample_list = [i for i in range(sample_num)]
        sample_list = random.sample(sample_list, sample_num)
        sample = train_aug_x_cap[sample_list, :, :]
    else:
    ...
    train_aug_x_cap = sample 
    train_aug_y_cap = np.zeros(sample_num) + 1

    train_aug_x = np.concatenate((train_aug_x, train_aug_x_cap), axis=0)  
    train_aug_y =  np.concatenate((train_aug_y, train_aug_y_cap), axis=0)


    train_x = train_x.transpose((0, 2, 1)) 
    train_aug_x = train_aug_x.transpose((0, 2, 1))  
    ...
    if dal_mode:
        ...
        train_out_x = train_aug_x
        train_out_y = train_aug_y

        train_in_dat = dict()
        train_in_dat["samples"] = train_in_x
        train_in_dat["labels"] = train_in_y

        train_out_dat = dict()
        train_out_dat["samples"] = train_out_x
        train_out_dat["labels"] = train_out_y

        train_in_dataset = Load_Dataset(train_in_dat, configs)
        train_out_dataset = Load_Dataset(train_out_dat, configs)

        # # batch code used for Wadi.
        # train_in_loader = torch.utils.data.DataLoader(dataset=train_in_dataset, batch_size=configs.batch_size,
        #                                            shuffle=True, drop_last=configs.drop_last,
        #                                            num_workers=0)
        # # For out loader, we may want a larger batch. Use same batch size for simplicity.
        # train_out_loader = torch.utils.data.DataLoader(dataset=train_out_dataset, batch_size=configs.batch_size,
        #                                            shuffle=True, drop_last=configs.drop_last,
        #                                            num_workers=0)
        # Batch code for other datasets.
        train_in_loader = torch.utils.data.DataLoader(dataset=train_in_dataset, 
                                                      batch_size=int(round(configs.batch_size / (1.0 + configs.rate))), 
                                                      shuffle=True, drop_last=configs.drop_last, num_workers=0)

        train_out_loader = torch.utils.data.DataLoader(dataset=train_out_dataset, 
                                                       batch_size=int(configs.batch_size - int(round(configs.batch_size / (1.0 + configs.rate)))), 
                                                       shuffle=True, drop_last=configs.drop_last, num_workers=0)
        ...
        return train_in_loader, train_out_loader, val_loader, test_loader, test_anomaly_window_num
